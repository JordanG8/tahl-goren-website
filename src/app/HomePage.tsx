            ...
            {displayReviews.map((review, index) => (
              <div key={index} className="bg-surface border border-outline/10 p-8 flex flex-col gap-4 hover:shadow-lg transition-shadow duration-300">
                <div className="flex items-start justify-between">
                  ...
                  <svg viewBox="0 0 24 24" className="w-5 h-5 text-[#4285F4] flex-shrink-0" fill="currentColor">
                ...
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map(i => (
                    <span key={i} className="material-symbols-outlined text-base" style={{ fontVariationSettings: "'FILL' 1", color: i <= review.rating ? '#FBBC04' : '#e0e0e0' }}>star</span>
                  ))}
                </div>
                <p className="font-body text-sm text-secondary leading-relaxed line-clamp-4">&ldquo;{review.text}&rdquo;</p>
              </div>
            ))}
          </div>
        </div>
